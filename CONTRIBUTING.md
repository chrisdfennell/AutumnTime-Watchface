...
Thanks for your interest in improving Autumn! This is a golden-hour fall-foliage themed Garmin Connect IQ watch face for the Fenix 8 and tactix 8, written in [Monkey C](https://developer.garmin.com/connect-iq/monkey-c/). Contributions of all kinds are welcome — bug reports, layout/styling improvements, new indicators, device support, art/font assets, and documentation.
...
- `source/AutumnApp.mc` / `AutumnView.mc` — the app + watch face (all rendering is procedural in `onUpdate`).
- `resources/` — strings, settings, the base (454) bitmap fonts (`fonts/`), and the launcher icon.
- `resources-round-416x416/`, `resources-round-280x280/`, `resources-round-260x260/` — per-resolution bitmap font sets, wired up via device `resourcePath` entries in `monkey.jungle`.
- `fonts-src/` — source TTF fonts; `tools/gen_fonts.py` bakes them into the bitmap fonts for every panel size.

## Testing your changes

Autumn targets the **Fenix 8 and tactix 8** in both case sizes. Please verify your change on both before submitting:
...
- Match the existing style in `source/AutumnView.mc`: 4-space indentation, explicit type annotations on method signatures, and `private var` for fields.
...
- Complication / steps-bar palettes are the `C_*` constants at the top of the view; layout anchors are the percentage values in `onUpdate()`.
...
1. Fork the repo and create a topic branch off `main` (e.g. `feature/leaf-drift` or `fix/battery-label-clip`).
...
```
feat: add a second row of trees to the grove
